...
import { Camera, Upload, Image as ImageIcon, RotateCcw, ZoomIn, ZoomOut, Download, Trash2, Eye, Grid3x3, Sparkles } from 'lucide-react';
...
export default function ARCameraModule() {
  ...
  const loadSavedData = () => {
    try {
      const items = localStorage.getItem('ar_camera_items');
      const scenes = localStorage.getItem('ar_camera_scenes');
  ...
  const handleImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Array.from(files).forEach(file => {
      ...
      reader.onload = (e) => {
        ...
        localStorage.setItem('ar_camera_items', JSON.stringify(updated));
        showNotification('AR file uploaded successfully!', 'success');
      };
  ...
  const captureARScene = () => {
    ...
    localStorage.setItem('ar_camera_scenes', JSON.stringify(updated));
  ...
  const deleteUploadedImage = (imageUrl: string) => {
    if (window.confirm('Delete this AR file?')) {
      const updated = uploadedImages.filter(img => img !== imageUrl);
      setUploadedImages(updated);
      localStorage.setItem('ar_camera_items', JSON.stringify(updated));
      showNotification('AR file deleted', 'success');
    }
  };
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-4xl font-bold text-white">AR Camera</h1>
                <p className="text-blue-200 mt-1">Upload any file and visualize in AR with live camera</p>
              </div>
            </div>
            <div className="flex items-center space-x-2">
              <span className="px-3 py-1 bg-blue-500/30 text-blue-200 text-sm font-semibold rounded-full backdrop-blur-sm">
                {uploadedImages.length} AR Files
              </span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                ...
                {!cameraActive ? (
                  <div className="h-full flex flex-col items-center justify-center p-8 text-center">
                    ...
                    <div className="space-y-2 text-gray-300 text-sm max-w-md">
                      ...
                      <p><strong>Step 2:</strong> Upload any file (images, PDFs, etc.) on the right</p>
                      <p><strong>Step 3:</strong> Click a file to place it in AR view</p>
          ...
          <div className="space-y-6">
            <div className="bg-white/10 backdrop-blur-md rounded-xl border border-white/20 p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-white">AR Files</h3>
              </div>
              ...
              >
                ...
                <span className="font-semibold">Upload AR Files</span>
              </button>

              {uploadedImages.length === 0 ? (
                <div className="text-center py-8">
                  <ImageIcon className="w-12 h-12 text-gray-400 mx-auto mb-3" />
                  <p className="text-gray-300 text-sm">No AR files uploaded</p>
                  <p className="text-gray-500 text-xs mt-2">Upload any file to use in AR</p>
                </div>
              ) : (
            ...
            <div className="bg-gradient-to-br from-blue-500/20 to-cyan-500/20 rounded-xl p-4 border border-blue-400/30">
              ...
              <ul className="space-y-1 text-xs text-blue-200">
                <li>• Upload any files (images, PDFs, documents)</li>
                <li>• Click file to place in AR view</li>
        ...
        <div className="mt-6 bg-white/10 backdrop-blur-md rounded-xl border border-white/20 p-6">
          ...
          {savedScenes.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-300 text-lg font-medium">No scenes captured yet</p>
              <p className="text-gray-500 text-sm mt-2">Place AR items and click "Capture Screenshot"</p>
            </div>
          ) : (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {savedScenes.map((scene) => (
                <div key={scene.id} className="relative group bg-white/5 rounded-lg overflow-hidden hover:bg-white/10 transition-colors">
                  <img
                    src={scene.image}
                    alt={scene.item?.name || 'AR Scene'}
                    className="w-full h-40 object-cover"
                  />
                  <div className="p-3">
                    <p className="text-white text-sm font-medium truncate">{scene.item?.name || 'AR Scene'}</p>
